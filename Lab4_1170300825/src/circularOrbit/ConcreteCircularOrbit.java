...
//                E po = entry.getKey();
                for (Relation<E, E> rel : entry.getValue()) {
                    UncheckedException.assertTrue(poSet.contains(rel.getObjv()) && poSet.contains(rel.getObju()),
                            "relOf2TraObjs不满足循环不变性");
                }
            }
//            List<Track> sortedTracks = getSortedTrack();
//            Set<E> firstTrackObjSet = new HashSet<>(physicalObjectMap.get(sortedTracks.get(0)));
//            for (Relation<L, E> rel : relOfCobj2TraObj) {
//                UncheckedException.assertTrue(rel.getObju() == centralObject, "relOfCobj2TraObj relation.u不满足循环不变性");
//                UncheckedException.assertTrue(firstTrackObjSet.contains(rel.getObjv()), "relOfCobj2TraObj relation.u不满足循环不变性");
//            }
...
//        在关系表中删除该节点
    ...
    protected Map<E,Integer> getSingleSourceDistances(List<E> sources) {
        try {
            sources.stream().forEach(x -> UncheckedException.assertTrue(relOf2TraObjs.containsKey(x), "source" + x.toString() + "不存在"));
        } catch(UncheckedException e) {
            logger.error(e);
            throw e;
        }
    ...
    private class MyIterator<E extends Comparable> implements Iterator<E> {
        ...
        @Override
        public E next() {
            if(ite>=size) throw new NoSuchElementException();
