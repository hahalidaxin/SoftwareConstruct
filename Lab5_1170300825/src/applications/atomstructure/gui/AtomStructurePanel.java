...
public class AtomStructurePanel extends JPanel {
  private static Logger logger = Logger.getLogger(AtomStructurePanel.class.getName());
...
//        inputframe = null;
  ...
  private void buttonRemovePOMousePressed(MouseEvent e) {
    try {
      double rmRadius = Double.parseDouble((String) this.comboBoxRemovePO.getSelectedItem());
  ...
  private void buttonTransitMousePressed(MouseEvent e) {
    try {
      double source = Double.parseDouble((String) this.comboBoxSource.getSelectedItem());
      double target = Double.parseDouble((String) this.comboBoxTarget.getSelectedItem());
      if (source == target) {
        JOptionPane.showMessageDialog(this, "源轨道与目标轨道不能相同");
        logger.error("源轨道与目标轨道不能相同");
      } else {
